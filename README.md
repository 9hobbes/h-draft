...
_한스펠 관련은 컨텍스트 메뉴에서 거의 다 처리가 가능해 보여서, 상단 메뉴는 텍스트 에디터로서 어디까지_
_기능을 지원할 것인가하고 관련이 있을거 같은데,_
_https://react-rte.org/demo 이 데모가 보통 생각하는 웹 텍스트 편집기의 표준 형태를 보여준다고 생각됨._
_여기에 기본 한스펠을 호출하는 버튼이 있어여 하고, 명세로 보면 ZEN모드(?) 지원 같은 모드 변환 기능이나_
_타이프라이터 스크롤 온/오프 기능을 널을 수 있다고 보는데, 타이프라이터 스크롤은 항상 켜는 것으로_
_생각하면 빼도 좋다고 생각함._

_요약하자면, 텍스트 에디터로서의 기본 기능 + 필요한 기능 호출 버튼이 들어가야 하는데, 최소한만_
_남기는 것이 이 프로젝트가 추구하는 바와는 맞을거라 생각함._
...
_공부 관련해서는 개인이 개발하면서 알아서 하면 되는 것이니까 논외로 하고,_
_기능 구현은 모든 요소가 처음 해보는 거라 예측 자체가 힘들기때문에_
_예기한데로 백로그 작성하고, 그것을 각자가 받아가서 1주일이나 10일 단위로_
_구현 결과를 공유하고 계획을 수정해나가면 될듯함. 마일스톤도 그 기준으로 잡고_

_그중 가장 우선순위가 높은 것은 일렉트론 앱으로 만들기 같은데, 이게 전제가 되야_
_한스펠 호출과 로컬 DB 관련 기능이 구현이 될 수 있을거라고 봄. 그 이후에는_
